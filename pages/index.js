...
export default function Home({redirectionUrl, sessionToken, user}) {
            ...
            <div className="flex flex-grow flex-col items-center justify-end self-start p-6">
                ...
                <a href={redirectionUrl}
                   className="group flex w-full m-6 border p-4 rounded-xl bg-darkBackground-900 gap-x-6 items-center hover:border-white">
                    ...
                    <div className="grow">
                        <p className="text-lg text-brandLight font-bold">Generate Images Using AI</p>
                        <p className="text-base">Connect your HandCash Account</p>
                    </div>
                </a>
            </div>
        )
    ...
    return (
        <div className="flex flex-grow flex-col items-center justify-end self-start p-6">
            ...
            <div className="flex items-center gap-x-1 group" style={{
                ...
                <form
                    className={"flex px-4 h-8 items-center rounded-full border bg-gradient-to-r from-brandNormal to-brandDark hover:opacity-90 text-sm font-semibold hover:cursor-pointer" + (imageResult?.status === 'pending' ? ' animate-pulse' : '')}
                    style={{
                ...
                >
                    ...
                    <button type="submit" className="ml-3" disabled={imageResult?.status === 'pending'}>Generate Image ($0.04)</button>
                </form>
            </div>
            ...
            {imageResult.status === 'created' &&
                <div
                    ...
                    <div>
                        <img src={imageResult.imageUrl} alt="Stylized image" className="w-full" />
                    </div>
                </div>
            }
            {imageResult.status === 'error' &&
                <div
                    ...
                    <div>
                        <p className="text-lg text-white font-bold">Image creation failed</p>
